...
function setup_testing_environment() {
  ...
  testing_env_build=$(docker run -v $project_directory:/project --rm --link decompose-docker-common-testing:docker docker sh -c "cp -rL /project /project-no-symlinks && docker build -t tester /project-no-symlinks/test/bats/fixtures/docker-compose/.")
  [ "$?" == "1" ] && echo "$testing_env_build"
}
...
function teardown_testing_environment() {
  ...
  testing_env_cleanup=$(docker rm -f decompose-docker-common-testing)
